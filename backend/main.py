...
from analyzer import analyze_diff_with_gemini, format_comment
...
async def process_pull_request(event: PREvent):
    """Fetch PR diff, analyze with Gemini, post comment on GitHub."""
    ...
    print(f"[PR #{event.number}] Diff fetched ({len(diff)} chars)")

    # Step 2: Analyze with Gemini AI
    findings = await analyze_diff_with_gemini(diff, event.number)

    # Step 3: Format and post comment on GitHub PR
    comment = format_comment(findings, event.number)
    await client.post_pr_comment(event.repo_full_name, event.number, comment)
    print(f"[PR #{event.number}] ✅ Analysis complete — comment posted!")
